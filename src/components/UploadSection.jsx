...
export function UploadSection({
  ...
  const confidence =
    prediction?.confidence != null ? Number(prediction.confidence) : null;
  const confPercent =
    confidence != null
      ? (confidence > 1 ? confidence : confidence * 100).toFixed(2)
      : null;
  const hasNodule =
    predictedClass.toLowerCase().includes("nodule") &&
        ...
        <div className="p-3 mb-4 bg-[#1F2937] border border-gray-700 shadow-none rounded-lg shrink-0 w-full flex flex-col">
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              <Button
                className="btn-choosefile cursor-pointer flex-1 bg-[#0EA5E9] hover:bg-[#0d96d4] h-8 text-xs"
                onClick={openFileDialog}
              >
                <Upload className="mr-1 h-3 w-3" /> Select Local Files
              </Button>
              <Button
                className="btn-choosefile cursor-pointer flex-1 bg-[#0038c6] hover:bg-[#002a94] h-8 text-xs"
                onClick={() => setIsLibraryOpen(true)}
              >
                <Cloud className="mr-1 h-3 w-3" /> Select Cloud Files
              </Button>
            </div>

            <Button
              className="btn-classify cursor-pointer w-full bg-[#14B8A6] hover:bg-[#10A39B] h-8 text-xs"
